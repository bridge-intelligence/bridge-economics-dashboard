...
1. File exceeds ~300-400 lines (TS/JS) or ~400-600 lines (Kotlin/Java) AND contains multiple responsibilities
...
1. `src/api/` for REST controllers
2. `src/service/` for business logic orchestration
3. `src/domain/` for pure domain logic (no IO)
4. `src/repo/` for DB access (Spring Data JPA repositories)
5. `src/events/` for Kafka consumers/producers + inbox/outbox
6. `src/config/` for Spring configuration classes
...
4. Avoid circular imports. Keep dependency direction: api -> service -> domain -> repo/events.
...
1. File names must reflect responsibility:
   - `PostingService.kt`, `LedgerController.kt`, `AccountRepository.kt`, `LedgerEventConsumer.kt`
2. Keep folders shallow. Prefer 1-2 levels deep.
3. Avoid "god" utilities. If `Utils.kt` grows, split it.
...
2. Lint/format checks pass (if repo has them): `./gradlew ktlintCheck`
...
### Purpose
bridge-economics-dashboard is a public-facing dashboard showing BRIDGE platform economics — tokenomics, reserves, TVL, supply feeds across the BRIDGE DLT, Ethereum/L2s, and Solana.

### Non-Goals
- No operator-only data.
- No user-specific balances.
- No regulator-specific views (bridge-corda-dashboards).
...
Data sources: bridge-oracle (prices), bridge-tokenization-register (supply), on-chain reads.
...
pnpm dev
pnpm build
kubectl port-forward -n bridge-service-stack svc/bridge-economics-dashboard 8092:80
```

## 19. Domain Rules

1. **Public data only** — no operator internals surface here.
2. **Real-time but cached**: 30s-5min TTL on data fetches.
3. **Source attribution**: every figure must show its source chain/service.

## 20. Forbidden Actions

1. Never show user-level balances.
2. Never show internal venue identities beyond public aggregates.
3. Never bundle API keys; all data fetched via public gateway endpoints.
